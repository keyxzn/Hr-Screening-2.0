...
export default function BulkHistoryPage() {
  ...
  const stats = {
    ...
    candidates:sessions.reduce((a, s) => a + s.totalCandidates, 0),
  };
        ...
        <div style={{ maxWidth: 1040, margin: "0 auto", padding: "24px 32px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
            {[
              ...
              { label: "Total Kandidat", val: stats.candidates, icon: Users,           color: "var(--accent)" },
            ].map((s) => (
          ...
          {sessions.length === 0 ? (
            <div className="card" style={{ textAlign: "center", padding: "80px 0" }}>
              ...
              <Link href="/candidates/bulk/new" className="btn btn-primary" style={{ display: "inline-flex", gap: 8 }}>
                <Upload size={13} /> Upload Kandidat Massal
              </Link>
            </div>
          ) : (
            <div className="card" style={{ overflow: "hidden" }}>
              ...
              {sessions.map((s, i) => {
                ...
                const Icon = cfg.icon;
                const completePct = s.totalCandidates > 0 ? Math.round((s.completed / s.totalCandidates) * 100) : 0;
                return (
                  <div key={s.id} style={{
                    padding: "16px 20px",
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: 11.5, color: "var(--text3)" }}>
                          <span style={{ color: "var(--accent)", fontWeight: 700 }}>{s.completed}</span> / {s.totalCandidates} kandidat
                        </span>
                        {s.failed > 0 && (
                    ...
                    <Link href="/candidates" style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 600, color: "var(--accent)", textDecoration: "none", whiteSpace: "nowrap" }}>
                      Lihat kandidat <ChevronRight size={13} />
                    </Link>
                  </div>
                );
              })}
