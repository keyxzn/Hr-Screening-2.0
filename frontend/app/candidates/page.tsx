...
export default function CandidatesPage() {
  ...
  async function downloadBulk(riskFilter: string) {
    ...
    if (targets.length === 0) {
      alert(`Tidak ada kandidat ${riskFilter === "all" ? "" : `risiko ${riskFilter} `}yang sudah selesai screening.`);
      setDlOpen(false);
      return;
    }
    setDlLoading(true);
    ...
    try {
      ...
      function buildPage(cand: Row, isFirst: boolean) {
        ...
        doc.text(safe(cand.full_name), margin + 5, y + 9);
        doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(71, 85, 105);
        doc.text(`Email: ${safe(cand.email)}`, margin + 5, y + 16);
        doc.text(`Telp: ${safe(cand.phone ?? "-")}`, margin + 5, y + 21);
        doc.text(`ID: ${safe(cand.id.slice(0, 8))}...`, margin + 5, y + 26);
        ...
        line(margin, y + 2, W - margin, y + 2, ...rc);
        y += 7;
        const summaryLines = doc.splitTextToSize(safe(report.ai_summary ?? "Tidak ada ringkasan."), W - margin * 2);
        ...
        if (flagged.length === 0) {
          ...
          flagged.forEach((f: any, i: number) => {
            ...
            doc.text(`${i + 1}. [${safe(f.platform ?? "").toUpperCase()}] ${safe(f.category ?? "")}`, margin + 4, y + 6);
            doc.setFillColor(...sevC); doc.roundedRect(W - margin - 22, y + 2, 18, 6, 1, 1, "F");
            doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(255, 255, 255);
            doc.text(safe(f.severity ?? "").toUpperCase(), W - margin - 13, y + 6.5, { align: "center" });
            doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(71, 85, 105);
            const snippet = doc.splitTextToSize(safe(f.content_snippet ?? ""), W - margin * 2 - 10);
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setFont("helvetica", "normal"); doc.setFontSize(7); doc.setTextColor(148, 163, 184);
        doc.text("HRCheck — AI Recruitment Platform · Dokumen Rahasia", margin, 291);
      ...
      doc.save(`HRCheck_Bulk_${riskSlug[riskFilter]}_${new Date().toISOString().slice(0, 10)}.pdf`);
  ...
  const DL_OPTIONS = [
    { key: "all",      label: "Semua Kandidat", count: rows.filter(r => r.report?.status === "completed").length },
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 820 }}>
                  <thead>
                    <tr style={{ background: "var(--bg3)", borderBottom: "1px solid var(--border)" }}>
                      {["Kandidat", "Status", "Risiko", "Keputusan HR", "Dinilai Oleh", "Tanggal", ""].map(h => (
